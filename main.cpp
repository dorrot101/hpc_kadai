...
int main(const int argc, const char** argv)
{
	...
	if (exercise == 3)
	{
		...
		for (int k = 0; k < loop; k++)
		{
			...
			for (int i = 0; i < s; i++)
			{
				//計算 yに書き込み
				const float v = x.data[i];
				const float v_3 = v * v * v;
				const float v_5 = v_3 * v * v;

				y.data[i] = 3.f * ((1 + v) * (v_5 + v_3) + 1);
			}
			t.end();
	...
	if (exercise == 4)
	{
		...
		for (int k = 0; k < loop; k++)
		{
			...
			{
				//計算 ansに書き込み
				ans.data[i] = x.data[i] * (M_PI + sqrt(5) + pow(0.5 , 2));
			}
		...
		for (int k = 0; k < loop; k++)
		{
			...
			t.start();
			const int s = x.rows * x.cols;
			const float result = (M_PI + sqrt(5) + pow(0.5 , 2));
			//XXXXXXXX //定数値を計算
			for (int i = 0; i < s; i++)
			{
				//計算 yに書き込み
				y.data[i] = result * x.data[i];
			}
			t.end();
			//std::cout << "after : time: " << t.getLastTime() << " ms" << std::endl;
		}
	...
	if (exercise == 5)
	{
		...
		for (int k = 0; k < loop; k++)
		{
			...
			for (int j = 0; j < x.rows; j++)
			{
				...
				{
					//計算 ansに書き込み
					ans.data[j * x.rows + i]= x.data[j * x.rows + i] / u_PI;
				}
			}
			t.end();
		...
		for (int k = 0; k < loop; k++)
		{
			...
			for (int j = 0; j < x.rows; j++)
			{
				...
				{
					//計算
					y.data[j * x.rows + i]= x.data[j * x.rows + i] * u_PI_reciprocal;
				}
			}
			t.end();
		...
		for (int k = 0; k < loop; k++)
		{
			...
			{
				//計算
				y.data[i]=x.data[i] * u_PI_reciprocal;
			}
			t.end();
			//std::cout << "after : time: " << t.getLastTime() << " ms" << std::endl;
		}
	...
	if (exercise == 6)
	{
		...
		for (int k = 0; k < loop; k++)
		{
			...
			for (int j = 0; j < ans.rows; j++)
			{
				...
				{
					int coord = j * ans.rows + i;
					//計算 ansに書き込み
					ans.data[coord]=(a.data[coord]/b.data[coord])*(c.data[coord]/d.data[coord]);
				}
			}
			t.end();
			//std::cout << "before: time: " << t.getLastTime() << " ms" << std::endl;
		...
		for (int k = 0; k < loop; k++)
		{
			...
			for (int j = 0; j < ret.rows; j++)
			{
				for (int i = 0; i < ret.cols; i++)
				{
					int coord = j * ans.rows + i;
					//計算 retに書き込み
					ret.data[coord]=(a.data[coord]*c.data[coord])/(b.data[coord]*d.data[coord]);
				}
			}
			t.end();
			//std::cout << "after : time: " << t.getLastTime() << " ms" << std::endl;
	...
	if (exercise == 7)
	{
		...
		for (int j = 0; j < loop; j++)
		{
			...
			{
				//計算（retに入れる）
				ret.data[i] = kadai6(x.data[i], pow_n);
			}
			t.end();
		}
